...
This skill describes the preferred target shape when the repository is using, or intentionally moving toward, modular architecture. It does not require immediate decomposition of a very small or single-module scaffold when the simpler structure still fits the current scope.
